...
# Plots
sk_why <- ggplot(sk, aes(y = bias_sk, x = dev_skew, fill = n, colour = n)) + geom_point(size = 2) + labs(y = TeX("$Bias_{\\Delta sk}$ (small-sample estimator)"), x = TeX("Absolute Deviation from Normality $ \\left( \\left| ( sk_{g1} - 0 ) + ( sk_{g2} - 0 ) \\right| \\right)$"), fill = "Sample Size", colour = "Sample Size") + theme_classic() + scale_fill_viridis_c() + scale_colour_viridis_c()

ku_why <- ggplot(ku, aes(y = bias_ku, x = dev_kurt, fill = n, colour = n)) + geom_point(size = 2) + labs(y = TeX("$Bias_{\\Delta ku}$ (small-sample estimator)"), x = TeX("Absolute Deviation from Normality $ \\left( \\left| ( ku_{g1} - 3 ) + ( ku_{g2} - 3 ) \\right| \\right)$"), fill = "Sample Size", colour = "Sample Size") + theme_classic() + scale_fill_viridis_c() + scale_colour_viridis_c()
