...
## Key Functions ----
###------------------------------------------------------------------------###
#| @title calc.skewness: Calculate skewness and its sampling variance
#| @description Function to calculate skewness and its sampling variance
#| @param x Numeric vector of data
#| @param output Character string specifying whether to return the skewness estimate ("est") or its sampling variance ("var")
#| @return Skewness estimate or its sampling variance
#| @examples
#| calc.skewness(x = c(1, 2, 3, 4, 5), output = "est")
#| calc.skewness(x = c(1, 2, 3, 4, 5), output = "var")
...
#| @title sim_skew: Simulate Pearson distribution with specified moments
...
#| @examples
#| sim_skew(n = 50, params = params[i,], nsims = 1000)

sim_skew <- function(params, nsims = nsims) {
...
for(i in 1:nrow(params_all)) {
           params <- params_all[i,]
  result_skewness <- rbind(result_skewness, sim_skew(params = params, nsims = nsims))
